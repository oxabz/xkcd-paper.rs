...
use image::{DynamicImage, Rgba, Pixel, GenericImageView, GenericImage};
...
async fn main() {
    ...
    // Load
    let img = ImageReader::new(Cursor::new(image)).with_guessed_format().unwrap().decode().unwrap();
    ...
    img.resize((img.width()as f32 * ratio) as u32, (img.height()as f32 * ratio) as u32, FilterType::Triangle);

    //Composition
    let mut canva = DynamicImage::new_rgba8(width as u32, height as u32);
    canva.copy_from(&img, (width as u32-img.width())/2, (height as u32-img.height())/2);
    ...
    canva.pixels_mut().for_each(|pix|{
        ...
        *pix = fg.map2(&bg, |x,y|(x as f32 * grey+ y as f32 * (1.0-grey))as u8);
    });
    let canva = DynamicImage::ImageRgba8(canva);
    let mut image: Vec<u8> = Vec::new();
    canva.write_to(&mut image, image::ImageOutputFormat::Png).unwrap();
    ...
    match child.wait().await{
        Ok(code) => {
            if !code.success() {
                eprintln!("Error setting the wallpaper : feh exited with code {}", code.code().unwrap());
                return;
            };
        }
        Err(e) => {
